...
# Dice Loss 계산 함수
def dice_loss(pred, label, smooth=1e-6):
    """
    Dice Loss
    Args:
...
def combined_bce_dice_loss(pred, label, class_weights, smooth=1e-6):
    """
    ...
    dice_score = (2.0 * intersection + smooth) / (union + smooth)
    dice_loss = 1.0 - dice_score  # Dice Coefficient를 1에서 뺌 (Loss)
    return torch.mean(dice_loss) * 0.5 + bce_loss * 0.5
...
class CombinedBCEFocalLoss(nn.Module):
    def __init__(self, class_weights, focal_weight=0.5, gamma=2.0, alpha=0.8, epsilon=1e-9):
        """
        BCE Loss와 Focal Loss를 결합한 손실 함수 클래스.
        ...
        Args:
            ...
            epsilon (float): 수치 안정성을 위한 작은 값.
        """
        super(CombinedBCEFocalLoss, self).__init__()
    ...
    def forward(self, pred, label):
        ...
        # BCE Loss 계산
        bce_loss = F.binary_cross_entropy_with_logits(
            pred, label, weight=self.class_weights, reduction='none'
        )
        bce_loss = bce_loss.mean()
        ...
        label = label.float()
        y_pred = torch.sigmoid(pred)  # Focal Loss에도 Sigmoid 적용
        ce = -label * torch.log(y_pred + self.epsilon)  # Cross-Entropy
        weight = (1 - y_pred) ** self.gamma  # Focal Loss 가중치 계산
