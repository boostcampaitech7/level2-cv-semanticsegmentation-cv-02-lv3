...
# 2. PyTorch 모델 정의 (Keras 모델과 동일한 구조를 PyTorch로 수동 구현)
...
class DuckV2Conv2DBlock(nn.Module):
    def __init__(self, input_channel, filters, size):
        ...
        self.x4_block = ResNetConv2DBlock(input_channel, filters)
        self.x5_block = ResNetConv2DBlock(input_channel, filters)
...
# 4. 가중치를 PyTorch 모델에 수동으로 로드
with torch.no_grad():
    pytorch_model.conv1.weight.copy_(torch.tensor(keras_weights[0]))
    pytorch_model.conv1.bias.copy_(torch.tensor(keras_weights[1]))
