...
from loss import DiceLoss, TverskyLoss, FocalLoss, BCEWithIoUAndSSIM, FocalLossWithIoUAndSSIM, CombinedBCEFocalLoss
from dataset import *
import numpy as np
...
def get_loss_function(loss_type, alpha=0.7, gamma=2.0):
    ...
    #     return TverskyLoss()
        ...
        return CombinedBCEFocalLoss(class_weights, focal_weight=0.5, gamma=2.0, alpha=0.7, epsilon=1e-9)
    else:
...
def objective(trial):
    ...
    start = time.time()
    loss_type = trial.suggest_categorical('loss_type', ["BCE", "Dice", "Focal", "BCE_Focal"]) #" Hybrid_BCE", "Hybrid_Focal"
    ...
    weight_decay = trial.suggest_float('weight_decay', 1e-5, 1e-5)
    ...
    model = DUCKNet(img_height=512, img_width=512, input_channels=3, out_classes=29, starting_filters=30).to(device)
